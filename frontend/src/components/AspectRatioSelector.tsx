...
const AspectRatioSelector = ({value, onChange} : {value: AspectRatio; onChange: (ratio:AspectRatio)=> void}) => {
  ...
  return (
    <div className='space-y-3 dark'>
        <label className="block text-sm font-medium font-zinc-200">Aspect Ratio</label>
        <div className="flex flex-wrap gap-2">
            {aspectRatios.map((ratio)=>{
            const selected = value === ratio;
            return (
                <button key={ratio} type='button' onClick={()=>onChange(ratio)}
                className={`flex items-center gap-2 rounded-md border px-5 py-2.5 text-sm tranition border-white/10 ${selected ? 'bg-white/10' : 'hover:bg-white/6'}`}>
                    {iconMap[ratio]}
